...
public class LeetCode_1889 {
    public int minWastedSpace(int[] packages, int[][] boxes) {
        ...
        // packages前缀和
        long[] pre = new long[packages.length + 1];
        for(int i = 1; i <= n; i++){
        ...
        for(int[] box : boxes){
            ...
            for(int y : box){
                ...
                int index = search(packages, y, pt);
                long curValue = getSection(pre, prevIndex, index);
                total += ((index - prevIndex) * (long)y) - curValue;
                prevIndex = index;
            }
    ...
    int search(int[] nums, int target, int low){
     ...
     * 返回left到right的区间和
     * */
    long getSection(long[] pre, int left, int right){
    ...
    public static void main(String[] args){
        ...
        LeetCode_1889 solution = new LeetCode_1889();
        int ans = solution.minWastedSpace(packages, boxes);
