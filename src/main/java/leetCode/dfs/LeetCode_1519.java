 ...
 * 树的根节点为节点 0 ，树上的每一个节点都有一个标签，也就是字符串 labels 中的一个小写字符
 * （编号为 i 的 节点的标签就是 labels[i] ）
...
public class LeetCode_1519 {
    ...
    int[] dfs(int level, boolean[] visited){
        visited[level] = true;

        int[] count = new int[26];
        count[labels.charAt(level) - 'a']++;

        for(int next : graph[level]){
            if(!visited[next]){
        ...
        ans[level] = count[labels.charAt(level) - 'a'];
        return count;
    }
